...
assert(
  html.includes('function syncCatanViewportMetrics'),
  'Catan should sync visualViewport height/offset for mobile landscape webviews'
);
assert(
  html.includes('--catan-vh'),
  'CSS should use a JS-backed viewport height variable instead of raw 100dvh only'
);

const appRule = html.match(/#app\s*\{[\s\S]*?\n\s*\}/)?.[0] || '';
assert(appRule.includes('var(--catan-vh, 100dvh)'), 'base #app height should use corrected viewport height');
...
assert(
  ...
  'visualViewport resize should resync Catan viewport metrics'
);

console.log('PASS catan landscape safe viewport');
