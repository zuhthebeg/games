...
class MultiplayerUI {
    constructor(options) {
        ...
        this.getRoomConfig = options.getRoomConfig || null;  // Optional: returns extra createRoom config
    ...
    _renderLobby() {
        ...
        this.container.innerHTML = `
            <div class="mp-ui">
                <div class="mp-screen">
                    ...
                    <div style="font-size:1.3rem;font-weight:700;margin-bottom:12px;">👥 ${this.gameName} 대전</div>
                    
                    <!-- 방 목록 (메인 영역) -->
                    <div class="mp-card" style="padding:16px;flex:1;display:flex;flex-direction:column;max-height:40vh;">
                        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;">
                            <span style="font-weight:700;font-size:0.95rem;">🏠 공개 방</span>
                            <button class="mp-btn" data-action="refresh-rooms" style="width:auto;padding:4px 10px;font-size:0.75rem;">🔄 새로고침</button>
                        </div>
                        <div id="mp-room-list" style="flex:1;overflow-y:auto;-webkit-overflow-scrolling:touch;">
                            <div style="text-align:center;color:var(--mp-muted);padding:20px;">로딩 중...</div>
                        </div>
                    </div>
                    
                    <!-- 하단 액션 영역 -->
                    <div class="mp-card" style="padding:14px;">
                        <!-- 코드 입장 -->
                        <div class="mp-field" style="margin-bottom:12px;">
                            <label class="mp-label" style="font-size:0.8rem;margin-bottom:6px;">코드로 입장</label>
                            <div style="display:flex;gap:8px;">
                                <input type="text" class="mp-input" id="mp-room-code" 
                                       placeholder="방 코드 입력" maxlength="6"
                                       style="flex:1;text-align:center;letter-spacing:3px;text-transform:uppercase;font-weight:600;">
                                <button class="mp-btn primary" data-action="join" style="width:auto;padding:12px 20px;">입장</button>
                                <button class="mp-btn outline" data-action="qr-scan" style="width:auto;padding:12px 16px;font-size:1.3rem;">📷</button>
                            </div>
                        </div>
                        ...
                        ${isLoggedIn ? `
                            <div style="display:flex;gap:8px;">
                                <input type="text" class="mp-input" id="mp-room-name" 
                                       ...
                                       style="flex:1;padding:10px 14px;font-size:0.9rem;">
                                <input type="checkbox" id="mp-public" checked style="display:none;">
                                <button class="mp-btn primary" data-action="create" style="width:auto;padding:10px 18px;white-space:nowrap;">+ 방 만들기</button>
                            </div>
                        ` : `
                            <button class="mp-btn primary" onclick="SharedWallet.showLogin()" style="margin-bottom:8px;">🔑 로그인하고 방 만들기</button>
                        `}
                        <button class="mp-btn" data-action="random" style="margin-top:8px;">🎲 랜덤 매칭</button>
                    </div>
                </div>
            </div>
        `;
        ...
        const refreshBtn = this.container.querySelector('[data-action="refresh-rooms"]');
        if (refreshBtn) refreshBtn.onclick = () => this._loadRoomList();
    ...
    async _loadRoomList() {
        ...
        try {
            ...
            if (rooms.length === 0) {
                listEl.innerHTML = `<div style="text-align:center;color:#64748b;padding:24px;font-size:0.9rem;">
                    🏜️ 대기 중인 방 없음
                </div>`;
                return;
            }
            ...
            listEl.innerHTML = rooms.map(r => {
                ...
                return `<div class="mp-room-item" data-room-id="${r.id}" style="display:flex;justify-content:space-between;align-items:center;padding:8px 12px;background:#1e293b;border-radius:8px;margin-bottom:4px;cursor:pointer;border:1px solid #334155;">
                    <div style="display:flex;align-items:center;gap:8px;">
                        <span style="font-size:0.9rem;font-weight:600;color:#fff;">${roomName}</span>
                        <span style="font-size:0.75rem;color:#64748b;">👥${playerCount}/${maxP}</span>
                    </div>
                    <button class="mp-btn" style="padding:4px 12px;font-size:0.8rem;">입장</button>
                </div>`;
            }).join('');
    ...
    _roomJoinUrl(roomCode) {
        return `${this._inviteBaseUrl()}/?room=${encodeURIComponent(roomCode || '')}`;
    }

    _renderWaitingRoom(roomCode) {
        this.currentView = 'waiting';
        const joinUrl = this._roomJoinUrl(roomCode);
    ...
    _renderResult(options) {
        ...
        // Custom actions: [{ label, primary, handler }]
        // If not provided, fall back to default rematch/exit
        const customActions = options.actions;
        let buttonsHTML;
        
        if (customActions && customActions.length > 0) {
            buttonsHTML = customActions.map((a, i) => 
                `<button class="mp-btn ${a.primary ? 'primary' : 'outline'}" data-custom-action="${i}">${a.label}</button>`
            ).join('');
        } else {
            const showRematch = options.showRematch !== false;
            buttonsHTML = (showRematch ? '<button class="mp-btn primary" data-action="rematch">🔄 리매치</button>' : '')
        ...
        overlay.innerHTML = `
            <div class="mp-result-content">
                ...
                <div class="mp-btn-group">${buttonsHTML}</div>
            </div>
        `;
        ...
        if (customActions && customActions.length > 0) {
            customActions.forEach((a, i) => {
                const btn = overlay.querySelector(`[data-custom-action="${i}"]`);
                if (btn) btn.onclick = () => { this._hideResult(); a.handler(); };
            });
        } else {
            const rematchBtn = overlay.querySelector('[data-action="rematch"]');
            if (rematchBtn) rematchBtn.onclick = (e) => this._handleRematch(e.target);
            const exitBtn = overlay.querySelector('[data-action="exit"]');
            if (exitBtn) exitBtn.onclick = () => this._handleExit();
        }
    }
    ...
    async _handleCreate(btn) {
        ...
        try {
            ...
            const playerState = this.getPlayerData ? this.getPlayerData() : null;
            const roomConfig = this.getRoomConfig ? this.getRoomConfig() : null;
    ...
    _shareLink(code) {
        const url = this._roomJoinUrl(code);
