...
(function () {
  ...
  MultiplayerLobby.prototype._connect = function () {
    ...
    ws.onPresence = function (n) {
      ...
      // 진행 중 상대 이탈(탭 닫기/튕김/잠깐 백그라운드=opponent_left 이벤트 없음):
      // 바로 방폭하지 말고 5초×3회(~15초) 재연결을 기다린 뒤, 그래도 인원 미회복이면 그때 종료.
      // WSClient가 그 사이 자동 재접속 시도. presence 회복되면 타이머 취소(잠깐 다른 창 봐도 안 터짐).
      // AI 좌석은 connection이 아니라 solo+AI(시작인원=1)에선 오탐 없음.
      if (self._inGame && !self._ended && self._startConns && n < self._startConns) {
        if (!self._leaveTimer) {
          self._leaveWaits = 0;
          var _tick = function () {
            if (!self._inGame || self._ended || (self._roster.connections || 0) >= self._startConns) { self._leaveTimer = null; self._setStatus(''); return; }
            self._leaveWaits++;
            if (self._leaveWaits >= 3) { self._leaveTimer = null; self._opponentGone(); return; }
            self._setStatus('⏳ 상대 연결 끊김 — 재접속 대기 중… (' + self._leaveWaits + '/3)');
            self._leaveTimer = setTimeout(_tick, 5000);
          };
          self._leaveTimer = setTimeout(_tick, 5000);
        }
      } else if (self._leaveTimer && n >= (self._startConns || 0)) {
        clearTimeout(self._leaveTimer); self._leaveTimer = null; self._setStatus('');
      }
    };
    ...
    ws.onNotice = function (d) {
      ...
      // 진행 중 액션 거절(서버 validateAction 실패) → 게임에 전달(보드 원복·사유 표시용)
      else if (d && d.type === 'error') self.onGameEvent('action_error', d);
    };
  ...
  MultiplayerLobby.prototype._renderLobby = function () {
    ...
    ov.querySelector('#mpl-start').onclick = function () {
      ...
      _cfg.fillAI = false;   // 일반 시작: 들어온 사람들로만. 빈자리 AI 안 채움.
    ...
    if (_soloBtn) _soloBtn.onclick = function () {
      ...
      var _cfg = {}; if (self._startCfg) { try { _cfg = self._startCfg() || {}; } catch (e) { _cfg = {}; } }
      _cfg.fillAI = true;    // 이 버튼만 빈자리 AI 채움.
