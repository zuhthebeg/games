  ...
  MultiplayerLobby.prototype._renderLobby = function () {
    ...
    ov.innerHTML =
      '<div class="mpl-card">' +
        ...
        '<button class="mpl-btn sec" id="mpl-solo-start" style="margin-top:2px">' + (this.onLocal ? '🎮 혼자 시작' : '🤖 혼자 시작 (빈 자리 AI)') + '</button>' +
    ...
    if (_soloBtn) _soloBtn.onclick = function () {
      ...
      var _cfg = {}; if (self._startCfg) { try { _cfg = self._startCfg() || {}; } catch (e) { _cfg = {}; } }
      self._ws.start(_cfg); self._setStatus('혼자 시작 — 빈 자리 AI로 채움');
    };
