...
class MultiplayerClient {
    ...
    async ensureAuth() {
        ...
        // 3. Fall back to anonymous auth (멀티 참여 = 실제 게임 액션이므로 여기선 즉시 생성)
        const mpGame = (location.pathname.split('/')[1] || 'root').slice(0, 32);
        const res = await this.request('POST', `/api/auth/anonymous?game=${encodeURIComponent(mpGame)}`);
...
class MultiplayerWSClient {
    ...
    /**
     * 의도적 나가기. close 전에 {type:'leave'}를 서버에 보내 '튕김'과 구분시킨다.
     * 서버는 진행 중이면 남은 사람에게 opponent_left를 통보한다. 그 후 leaveRoom과 동일.
     * @returns {Promise<{}>}
     */
    leave() {
        try {
...
// =============================================================================
// DO 기본값 게이트 (게이트 반전용). 설계: games-server/docs/do-default-flip-plan.md
// 우선순위: 명시 URL(?relay=do|rest) > 전역 킬스위치(localStorage relayKill) > 게임별 기본.
// 게임이 인라인 `relay==='do'` 게이트를 이 함수로 바꾸면 DO가 기본값이 된다(채택 전엔 무영향).
//   doDefault=true  → DO 기본 ON, ?relay=rest 가 탈출구 (Wave 게임)
//   doDefault=false → 기존 opt-in 유지, ?relay=do 만 DO (아직 안 넘긴 게임)
// 전역 즉시 롤백: localStorage.setItem('relayKill','1') 또는 URL ?relay=rest.
// =============================================================================
MultiplayerWSClient.relayDefaultOn = function (doDefault) {
    var p = null;
    try { p = new URLSearchParams(location.search).get('relay'); } catch (e) {}
    ...
    return doDefault === true;        // 게임별 기본(미지정=false=기존 opt-in 동작)
};
