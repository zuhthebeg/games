...
const SharedWallet = {
    ...
    showForgotPassword() {
        ...
        modal.innerHTML = `
            <div style="position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.8);z-index:10000;display:flex;justify-content:center;align-items:center;">
                <div style="background:#1e293b;border-radius:20px;padding:30px;max-width:400px;width:90%;text-align:center;color:#fff;">
                    <h2 style="margin-bottom:8px;">🔐 비밀번호 찾기</h2>
                    <p style="color:#94a3b8;font-size:0.85rem;margin-bottom:20px;">가입한 이메일을 입력하면 비밀번호 재설정 링크를 보내드립니다.</p>
                    <form id="sw-forgot-form">
                        <input type="email" id="sw-forgot-email" placeholder="이메일" required style="width:100%;padding:12px;margin-bottom:15px;border:2px solid #334155;border-radius:10px;font-size:1rem;background:#0f172a;color:#fff;">
                        <button type="submit" id="sw-forgot-btn" style="width:100%;padding:14px;background:linear-gradient(135deg,#3b82f6,#8b5cf6);border:none;border-radius:10px;color:white;font-size:1rem;font-weight:600;cursor:pointer;">재설정 링크 전송</button>
                    </form>
        ...
        document.getElementById('sw-forgot-form').onsubmit = async (e) => {
            ...
            try {
                const res = await fetch(`${SharedWallet.AUTH_API}/forgot`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
