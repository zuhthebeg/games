...
const SharedWallet = {
    ...
    async init(options = {}) {
        ...
        // 게스트 토큰 재사용만 (기존 방문자). 신규 계정 생성은 페이지 로드가 아니라
        // 첫 게임 액션(_save 클라우드 동기화 시점)까지 지연 — 크롤러가 페이지만 열어도
        // 익명 계정이 찍히던 봇 인플레이션 방지.
        this._autoGuestAllowed = options.autoGuest !== false;
        if (this._autoGuestAllowed && !localStorage.getItem('cocy_auth_token')) {
            const existingGuest = localStorage.getItem(this.GUEST_TOKEN_KEY);
            if (existingGuest) {
                localStorage.setItem('cocy_auth_token', existingGuest);
            }
        }
    ...
    async _save() {
        ...
        // 첫 게임 액션(골드 변동) 시점에 게스트 계정 지연 생성.
        // cloudSyncEnabled는 토큰이 있어야 _checkAuth에서 켜지므로, 토큰 없는 신규
        // 방문자는 여기서 계정을 만들고 직접 켠다.
        if (!this.cloudSyncEnabled && this._autoGuestAllowed && !localStorage.getItem('cocy_auth_token')) {
            await this._ensureGuestToken();
            if (localStorage.getItem('cocy_auth_token')) {
                this.cloudSyncEnabled = true;
                if (!this.user) this.user = { nickname: '게스트', isAnonymous: true };
            }
        }

        // 클라우드 저장
        if (this.cloudSyncEnabled) {
            let token = localStorage.getItem('cocy_auth_token');
            if (token) {
                // 토큰 만료 확인 → 만료됐으면 리프레시 후 재시도
                try {
                    ...
                    if (parts.length === 3) {
                        ...
                        const now = payload.userId ? Date.now() : Math.floor(Date.now() / 1000); // anon=ms, jwt=sec
                        if (exp < now) {
                            console.log('SharedWallet: Token expired, refreshing before save...');
                            const refreshed = await this._refreshToken();
                            if (refreshed) {
                                token = localStorage.getItem('cocy_auth_token');
                            } else {
                                console.log('SharedWallet: Token refresh failed, skip cloud save');
                ...
                try {
                    ...
                    // 401이면 토큰 리프레시 후 재시도 1회
                    if (res.status === 401) {
                        console.log('SharedWallet: PUT 401, refreshing token...');
                        const refreshed = await this._refreshToken();
                        if (refreshed) {
                            const newToken = localStorage.getItem('cocy_auth_token');
            ...
            if (tokenUserId) {
                ...
                if (dataRes.ok) {
                    ...
                    if (data.gold !== undefined) {
                        // 로컬 골드가 서버보다 크면 → 동기화 실패로 인한 차이 → 로컬 값 유지 + 서버에 업로드
                        const localGold = this.gold; // _loadLocal()에서 이미 로드됨
                        if (localGold > data.gold) {
                            console.log(`SharedWallet: Local gold (${localGold}) > Server gold (${data.gold}), keeping local & syncing up`);
                            this.gold = localGold;
                            // 서버에 로컬 값 업로드
                            this._save();
                        } else {
    ...
    // 통합 프로필(cocy.io/profile)로 이동. 토큰 저장소가 도메인별이라
    // 프래그먼트(#)로 토큰을 핸드오프한다 — 서버로 전송되지 않고, 프로필이 읽는 즉시 지운다.
    // 로그아웃 버튼은 여기서 뺐다: 로그아웃도 프로필 페이지에서 한다 (전 서비스 동일 UX).
    goProfile() {
        let url = 'https://cocy.io/profile/';
        try {
            const at = localStorage.getItem('cocy_auth_token');
            const rt = localStorage.getItem('cocy_refresh_token');
            if (at) url += '#at=' + encodeURIComponent(at) + (rt ? '&rt=' + encodeURIComponent(rt) : '');
        } catch (e) {}
        location.href = url;
    },
    ...
    // 게스트 토큰 확보 (없으면 anonymous 계정 생성). game 파라미터로 유입 게임 기록.
    async _ensureGuestToken() {
        try {
            const game = (location.pathname.split('/')[1] || 'root').slice(0, 32);
            const res = await fetch(`${this.AUTH_API}/anonymous?game=${encodeURIComponent(game)}`, { method: 'POST' });
            if (!res.ok) return;
            const data = await res.json();
    ...
    _fmtGold(g) {
        g = Number(g) || 0;
        if (g >= 1e9) return (g / 1e9).toFixed(2).replace(/\.?0+$/, '') + 'B';
        if (g >= 1e6) return (g / 1e6).toFixed(2).replace(/\.?0+$/, '') + 'M';
    ...
    _updateUI() {
        ...
        // 다국어 게임은 바 이름도 언어를 따라가야 한다 → SharedWalletConfig.gameName으로 덮어쓸 수 있게 둔다
        const gameName = cfg.gameName || this._gameNames[path] || path.toUpperCase() || 'GAMES';
        ...
        // 외부 모듈이 삽입할 수 있는 네비 슬롯 (game-rankings 등)
        const navExtra = this._navExtra || '';
        ...
        if (this.isLoggedIn) {
            ...
            bar.innerHTML = `
                ...
                <div id="sw-nav-extra" style="display:flex;align-items:center;gap:4px;">${navExtra}</div>
                <div style="display:flex;align-items:center;gap:6px;">
                    <span style="color:#ffd700;font-weight:700;font-size:13px;">💰 ${this._fmtGold(this.gold)}</span>
                    ...
                    <div style="position:relative;display:inline-block;">
                        ...
                        <div id="sw-profile-menu" style="display:none;position:absolute;right:0;top:calc(100% + 4px);background:#1e293b;border:1px solid rgba(255,255,255,0.1);border-radius:10px;min-width:140px;z-index:9999;overflow:hidden;box-shadow:0 8px 24px rgba(0,0,0,0.4);">
                            ...
                            <button onclick="SharedWallet.goProfile()" style="width:100%;text-align:left;background:none;border:none;color:#e2e8f0;padding:9px 12px;cursor:pointer;font-size:12px;display:flex;align-items:center;gap:7px;border-top:1px solid rgba(255,255,255,0.06);" onmouseover="this.style.background='rgba(255,255,255,0.07)'" onmouseout="this.style.background='none'">👤 내 프로필</button>
    ...
    addGold(amount) {
        ...
        this.gold += amount;
        console.log(`[SharedWallet.addGold] ${before} + ${amount} = ${this.gold}`);
        this._save();
        this._updateUI();
        // 디버그 토스트 (임시)
        this._debugToast(`💰 +${amount} → ${this.gold.toLocaleString()}G`);
        return this.gold;
    },
    ...
    // 임시 디버그 토스트 (화면 하단에 메시지 표시)
    _debugToast(msg) {
        const el = document.createElement('div');
        el.textContent = msg;
        el.style.cssText = 'position:fixed;bottom:60px;left:50%;transform:translateX(-50%);background:rgba(0,200,100,0.9);color:#000;padding:8px 16px;border-radius:20px;font-size:14px;font-weight:700;z-index:99999;pointer-events:none;transition:opacity 0.5s;';
