                ...
                <div style="position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.8);z-index:10000;display:flex;justify-content:center;align-items:center;">
                    <div style="background:#1e293b;border-radius:20px;padding:30px;max-width:400px;width:90%;text-align:center;color:#fff;">
                        ...
                        <p style="margin-top:15px;color:#94a3b8;font-size:0.9rem;">
                            계정이 없으신가요? <a href="https://www.cocy.io/register" target="_blank" style="color:#ec4899;">회원가입</a>
                        </p>
    ...
    _updateUI() {
        ...
        if (this.isLoggedIn) {
            ...
            bar.innerHTML = `
                ...
                <span>👤 ${nickname}</span>
                <span style="color:#4ecca3;font-size:12px;">☁️</span>
                <button onclick="SharedWallet.logout()" style="background:rgba(255,255,255,0.1);border:none;color:#fff;padding:4px 10px;border-radius:10px;cursor:pointer;font-size:12px;">로그아웃</button>
            `;
        } else {
            bar.innerHTML = `
                <span style="color:#ffd700;font-weight:600;">💰 ${this.gold.toLocaleString()}G</span>
                <button onclick="SharedWallet.showLogin()" style="background:linear-gradient(135deg,#ec4899,#8b5cf6);border:none;color:#fff;padding:6px 12px;border-radius:10px;cursor:pointer;font-size:12px;font-weight:600;">🔑 로그인</button>
            `;
        }
    },
...
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => SharedWallet.init());
} else {
    SharedWallet.init();
}
