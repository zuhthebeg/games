            ...
            if (payload.sub) {
                ...
                if (dataRes.ok) {
                    ...
                    if (data.gold !== undefined) {
                        // 로컬 골드와 클라우드 골드 중 큰 값 사용
                        // 로컬이 더 크면 클라우드에 업로드
                        const localGold = this.gold; // _loadLocal() 이미 실행됨
                        const cloudGold = data.gold;
                        if (localGold > cloudGold) {
                            this.gold = localGold;
                            // 로컬 → 클라우드 업로드 (비동기, 실패 무시)
                            const t2 = localStorage.getItem('cocy_auth_token');
                            if (t2) fetch(this.DATA_API, {
                                method: 'PUT',
                                headers: { 'Authorization': `Bearer ${t2}`, 'Content-Type': 'application/json' },
                                body: JSON.stringify({ gold: localGold, ...this._gameData })
                            }).catch(() => {});
                        } else {
                            this.gold = cloudGold;
                        }
