...
const SharedWallet = {
            ...
            if (payload.sub) {
                ...
                if (userRes.ok) {
                    ...
                    if (this.user) {
                        ...
                        this.user.nickname = fixedNick || this.user.email?.split('@')[0] || null;
                    }
                }
    ...
    // 로그인 모달 표시
    showLogin() {
        const isLineRush = window.location.pathname.includes('/linerush/');
        const loginLabel = isLineRush ? 'Login' : '로그인';
    ...
    _updateUI() {
        ...
        const homeBtn = isStandalone ? '' : `<a href="/" style="display:flex;align-items:center;justify-content:center;width:28px;height:28px;border-radius:8px;background:rgba(255,255,255,0.12);text-decoration:none;color:#fff;flex-shrink:0;transition:background 0.2s;" onmouseover="this.style.background='rgba(255,255,255,0.2)'" onmouseout="this.style.background='rgba(255,255,255,0.12)'">
        ...
        const isLineRush = location.pathname.includes('/linerush/');
        const loginLabel = isLineRush ? 'Login' : '로그인';
        const logoutLabel = isLineRush ? 'Logout' : '로그아웃';
        
        if (this.isLoggedIn) {
            const rawNick = this.user?.nickname || this.user?.email?.split('@')[0] || '유저';
            const nickname = this._normalizeNickname(rawNick) || this.user?.email?.split('@')[0] || '유저';
            bar.innerHTML = `
