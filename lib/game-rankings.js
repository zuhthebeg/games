...
(function (global) {
    ...
    // ISO2 국가코드 → 국기 이모지 (tileslider/script.js의 tsFlag와 동일 로직)
        ...
        const rows = rankings.length === 0
            ...
            : rankings.map((r, i) => {
                ...
                return `<tr style="${isMe ? 'background:rgba(255,215,0,0.1);font-weight:700;' : ''}">
                    ...
                    <td style="padding:6px 8px;">${flag}${nick}${isMe ? ' <span style="color:#ffd700;font-size:0.75em;">YOU</span>' : ''}</td>
    ...
    global.GameRankings = { submit, fetchRankings, showModal, closeModal, injectButton, injectNavButton, getUserInfo, GAME_CONFIG };
})(window);
