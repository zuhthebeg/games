...
(function (global) {
    ...
    function getUserInfo() {
        ...
        if (!userId) userId = localStorage.getItem('guestId') || ('guest_' + Math.random().toString(36).slice(2, 8));
        if (!nickname && typeof SharedWallet !== 'undefined') {
            try { nickname = SharedWallet.getNickname?.() || null; } catch { }
        }
    ...
    async function submit(gameId, data) {
        ...
        try {
            ...
            if (json.success) console.log(`[GameRankings] ${gameId} submitted`);
        } catch (e) {
    ...
    global.GameRankings = { submit, fetchRankings, showModal, closeModal, injectButton, GAME_CONFIG };
})(window);
