...
const oldKoreanMark = '\uce74\ud0c4';
assert(!html.includes(`${oldKoreanMark} 방`), 'old Korean mark should stay removed');
assert(!html.includes(`${oldKoreanMark} 스타일`), 'SEO description must not describe itself as old Korean mark style');
...
assert(!apps.includes(`"title": "${oldKoreanMark}"`), 'apps.json should not expose old Korean mark title');
